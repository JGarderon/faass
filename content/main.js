...
class TemplateConfEdit extends TemplateGeneric { 
  static idTemplate = 't_conf_edit';
}
customElements.define( 'conf-edit', TemplateConfEdit );
...
function goTo( part, ...rest ) {
  switch ( part ) {
    case 'error': 
      document.getElementById('content').innerHTML = `<error-detail detail="${rest[0]}"></error-detail>`;
      break 
    case 'before-refresh':
      document.getElementById('content').innerHTML = '<conf-get last="refresh"></conf-get>';
      break
    case 'refresh': 
      window.FaassConfiguration.refresh()
        .then( 
          _ => document.getElementById('content').innerHTML = `<conf-edit></conf-edit>` 
        )
        .catch( 
